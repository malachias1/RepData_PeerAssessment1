...
There are a total of `r nrow(activity)` observations 
in this data set.
...
count ranges from just a few steps to over 20,000 (`r dailyTotalMin` - 
`r dailyTotalMax`).  The mean number of steps
per day is `r sprintf("%.0f", dailyTotalMean)` and the median is `r dailyTotalMedian`.
...
Activity peaks at `r maxIntervalTimeStr` and rapidly diminishes thereafter. 
...
```
As mentioned above, the step column has a number of NAs in it, `r naCount` or 
`r sprintf("%.0f%%", naFraction*100)`. I will replace the NAs with the interval
mean of the data.
```{r, dailyTotalCalcsWithoutNAs}
...
The daily step range is unchanged and remains at `r dailyTotalMin`--`r dailyTotalMax`
The mean number of steps per day is, as expected, unchanged.
There is a slight change in the median from `r dailyTotalMedian` without 
imputation to `r sprintf("%.0f", dailyTotalMedianWithoutNAs)`.
```{r, dailyTotalHistWithoutNAs}
dailyTotals %>%
  ggplot(aes(x=total)) + 
  geom_histogram(binwidth=2500, colour='white') +
  xlab("Total Steps per Day") +
